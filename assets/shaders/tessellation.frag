...
layout (location = 2) in vec4 tes_col;
...
vec3 getNormalFromMap(vec2 texCoords)
{
    ...
    vec3 Q1  = dFdx(fragTessPosition);
    vec3 Q2  = dFdy(fragTessPosition);
    vec2 st1 = dFdx(texCoords);
    vec2 st2 = dFdy(texCoords);

    vec3 N   = normalize(fragTessNormal);
...
vec3 blinnPhongShading(vec4 textureColor) {
    vec3 norm = normalize(fragTessNormal);
    ...
    for (int light_i = 0; light_i < LIGHT_COUNT; light_i++) {
        ...
        vec3 light_dir = normalize(light.pos - fragTessPosition); // unit vector from light to fragment/pixel

        // calculate shadow depth via distance from pixel/fragment to light
        vec3 fragment_to_light = fragTessPosition - light.pos;
...
/*
        ...
        float light_distance = length(light.pos - fragTessPosition);
        ...
        vec3 camera_dir = normalize(fragTessPosition - camera_pos); // unit vector from camera to fragment/pixel
    ...
    // final color
    out_color = mix(tes_col, textureColor, texture_contribution);
...
void main() {
    ...
    vec2 texCoords = fragTessUV;

/*     if(hasHeightMap) {

        mat3 TBN = transpose(mat3(in_tan, in_bit, fragTessNormal));
        texCoords = ParallaxMapping(in_uv, normalize(TBN * (camera_pos - fragTessPosition)));
    ...
    if(noPBR) {//Anpassung für Objekte ohne PBR-Texturierung
        ...
        vec3 norm = fragTessNormal;
        if(hasAlbedoMap) {
            textureColor = texture(albedoMap, texCoords).rgba;//Gamma Korrektur?
        }
        if(hasNormalMap) {
            vec3 norm = calculateNormal(fragTessNormal, texCoords);
        }
...
/*     float metallic = 0.0;
    ...
    vec3 N = normalize(norm);
    vec3 V = normalize(camera_pos - fragTessPosition);
    ...
    for (int i = 0; i < LIGHT_COUNT; ++i) {
        ...
        vec3 L = normalize(light.pos - fragTessPosition);
